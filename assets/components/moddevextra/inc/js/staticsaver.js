var StaticSaver = function(config) {
    Ext.onReady(function(){
        ...
        if (MODx.config['moddevextra.static_default'] == 1) {
            isStatic.setValue(true);
        }

        var setValue = function(placeholder, value) {
            var new_value = '';
            if (MODx.config['moddevextra.static_file_extension'] !== '') {
                new_value = value + '.' + MODx.config['moddevextra.static_file_extension'];
            }
        ...
        var getRequest = function() {
            ...
            Ext.Ajax.request({
                url:  MODx.config['assets_url'] + 'components/moddevextra/connector.php',
                success: function(e){
                    var response = JSON.parse(e.responseText);
                    var value = response.result;
                    if (MODx.config['moddevextra.enable_rewrite'] || value == '1') {
                    ...
                    if (MODx.config['moddevextra.include_category'] == "1") {
