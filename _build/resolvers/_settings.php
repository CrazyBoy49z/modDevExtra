...
//		print_r($options['settings']);
		foreach ($settings as $k => $v) {
			if (!$newOpt = $modx->getObject('modSystemSetting', array('key' => $k))) {
				$newOpt = $modx->newObject('modSystemSetting');
				$newOpt->set('key', $k);

				if (!empty($v['namespace']))
					$newOpt->set('namespace', $v['namespace']);
				if (!empty($v['area']))
					$newOpt->set('area', $v['area']);
				if (!empty($v['xtype']))
					$newOpt->set('xtype', $v['xtype']);

			}

			$newOpt->set('value', $v['value']);
			$newOpt->save();
		}
		...
		foreach ($options['resources'] as $key => $val) {
			if (!empty($val['parent_uri'])) $parents_in[] = $val['parent_uri'];
			if (!empty($val['template_name'])) $templates_in[] = $val['template_name'];
		}
		...
		if ($template = $modx->getObject('modTemplate', array('templatename' => 'Главная'))) {
		...
		if (!empty($options['install_resources'])){
			...
			$modSetting = array('error404.html' => 'error_page', 'error403.html' => 'unauthorized_page', 'error503.html' => 'site_unavailable_page',);
